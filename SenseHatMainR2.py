...
def solveMac():
    ...
    try:
        ...
        URL3_FOUND = False
        string = "no host location"
        text(string, white, red)
    else:
        URL3_FOUND = True
        return response

def getHostLocation(response):
    global ApiKey2,URL4_FOUND, LAT, LON
...
def whereAmI():
    global URL3_FOUND,URL4_FOUND, LAT, LON, City, Country

    response = solveMac()
    if URL3_FOUND:
        dict1 = getHostLocation(response)
    if URL4_FOUND:
        dict2 = dict1['results']
        dict3 = dict2[1]
...
def getTimeZone():
    offsetTime = dt.datetime.fromtimestamp(0) - dt.datetime.utcfromtimestamp(0)
    offsetTime = str(offsetTime)
    split = offsetTime.split(":")
    offsetTime = split[0]
    offsetTime = int(offsetTime)
    return offsetTime

def getSunlight(offsetTime):
    global LAT, LON, eclipsed

    urlSun = 'http://api.sunrise-sunset.org/json?lat=%s&lng=%s&formatted=0' % (LAT,LON)
    try:
        sunlight = json.load(urllib2.urlopen(urlSun))
    except urllib2.URLError as e:
        string = "no sunset"
        text(string, white, red)
    else:
        sunlight = sunlight['results']
        sundown = sunlight['civil_twilight_end']
        split = sundown.split("+")
        sundown = split[0]
        eclipsed = dt.datetime.strptime(sundown, '%Y-%m-%dT%H:%M:%S')
        eclipsed = eclipsed + dt.timedelta(hours=offsetTime)

def getLightLevel():
    global eclipsed, ECLIPSED
...
def showRpiLocation():
    global LAT, LON, City, Country, ECLIPSED,URL4_FOUND
    ...
    if URL4_FOUND:
        ...
        Location = 'RPi at %s %s %s %s %s' % (City, Country,latStr,lngStr,lightStr) 
        text(Location, blue, black)
    else:
        string = "no Address"
        text(string,white,red)
...
def getNextOverhead():
    global URL4_FOUND,URL5_FOUND,City
    ...
    if URL5_FOUND:
        ...
        overhead = 'ISS in %s at %s for %s s' %(City,RiseTime,duration)
        text(overhead, yellow, black)
...
def processResponse(response):
    global url2, LAT, LON,City

    latitude = response['latitude']
    longitude = response['longitude']

    if (latitude == LAT and longitude == LON):
        text("Above",white,red)
        text(City, white,blue)
        text("Now",white,red)
    ...
    try:
        ...
        string = str("Sea %s %s %s" % ( latStr, lngStr, light ))
        text(string,white,blue)
        sense.clear(black)
    except urllib2.URLError as e:
        string = "no ESA response"
        text(string,white,red)
...
def text(string, colorT, colorB):
    
    sense.show_message(string, text_colour=colorT,back_colour=colorB)

def temperature(color):

    text("Tmp", color,black)
...
def pressure(color):
    ...
    text("Bar", color,black)
    pressure = sense.get_pressure()
    sense.show_message("%d    " % pressure, text_colour=color)
...
####
##Initialize: Get local data and twilight time
####

offsetTime = getTimeZone()
whereAmI()
getSunlight(offsetTime)
getLightLevel()
text(City, white,black)
line1 = dt.datetime.strftime(dt.datetime.now(), '%d %b %Y %H:%M')
text(line1, yellow,black)
text("eclipsed at:",red,black)
line2 = dt.datetime.strftime(eclipsed, '%H:%M')
text(line2, blue,black)
rainbow()
